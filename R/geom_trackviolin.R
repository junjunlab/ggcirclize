...
#' @usage NULL
#' @import ggplot2
#' @include geom_arcviolin.R
#' @export
GeomTrackviolin <- ggproto("GeomTrackviolin", GeomArcviolin,
                             ...
                             for (sec in seq_along(gp)) {
                               ...
                               if(scales == "fixed"){
                                 ...
                                 if(panel_params$x$is_discrete()){
                                   ...
                                   yAxis.params <- modifyList(list(yscale = range(data$ymin,data$ymax)),
                                                              yAxis.params)
                                 }else{
                                   new_panel_params$x.range <- range(tmp_data$xmin,tmp_data$xmax)
                                   yAxis.params <- modifyList(list(yscale = range(data$y - unique(data$width)/2,
                                 ...
                                 if(panel_params$y$is_discrete()){
                                   ...
                                   xAxis.params <- modifyList(list(xscale = range(data$xmin,data$xmax)),
                                                              xAxis.params)
                                 }else{
                                   new_panel_params$y.range <- range(tmp_data$ymin,tmp_data$ymax)
                                   xAxis.params <- modifyList(list(xscale = range(data$x - unique(data$width)/2,
                               ...
                               ptack <-
                                 ggproto_parent(GeomArcviolin, self)$draw_panel(
...
#' geom_trackviolin Function
#'
#' This is a custom geom function used to draw track violin plots.
#'
...
#' @param strip.label.space The spacing between track labels and track violin plots (default = 0.15).
...
#' @param space Specifies the spatial type for track violin plots (default = "free_x").
#' @param sector.gap The gap between track violin plots (default = 3).
#' @param draw_quantiles Whether to draw quantile lines (default = NULL).
#' @param quantiles.colour The color of quantile lines (default = "black").
#' @param quantiles.linewidth The line width of quantile lines (default = 1).
#' @param quantiles.linetype The line type of quantile lines (default = 1).
...
#' @param flipped_aes Whether to use flipped aesthetics (default = FALSE).
...
#' @import ggplot2
#' @export
geom_trackviolin <- function(mapping = NULL, data = NULL,
                             stat = "arcydensity", position = "dodge",
