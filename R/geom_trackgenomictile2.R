...
#' @export
GeomTrackgenomictile2 <- ggproto("GeomTrackgenomictile2", GeomArctile,

                                 required_aes = c("chr","gstart","gend","x","y",
                                   ...
                                   for (sec in seq_along(gp)) {
                                     ...
                                     if(nrow(tmp_data) > 0){
                                       ...
                                       if(add_link == TRUE){
                                         ...
                                         link_df <- unique(tmp_data[,c("chr","gstart","gend")])
                                         link_df <- link_df[order(link_df$gstart),]
                                         link_df$ht.x <- 1:nrow(link_df)
                                         link_df$ht.x.th <- rescale(link_df$ht.x,to = rev(range.th),
                                                                    from = c(min(link_df$ht.x) - unique(tmp_data$width)/2,
                                                                             max(link_df$ht.x) + unique(tmp_data$width)/2))
                                         ...
                                         vbline <- arcSegmentsGrob(xmin = link_df$ht.x,
                                                                   ...
                                                                   extend.xscale = extend.xscale,
                                                                   extend.yscale = 0,
                                                                   xscale = c(min(link_df$ht.x) - unique(tmp_data$width)/2,
                                                                              max(link_df$ht.x) + unique(tmp_data$width)/2))
...
#' Track Genomic Tile Plot
#'
#' This function creates a genomic tile plot using track-based visualization.
#'
...
#' @param add_link A logical value indicating whether to add links to tiles (default: FALSE).
...
#' @import ggplot2
#' @export
geom_trackgenomictile2 <- function(mapping = NULL, data = NULL,
