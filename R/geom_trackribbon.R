...
#' @export
GeomTrackribbon <- ggproto("GeomTrackribbon", GeomArcribbon,
                           ...
                           draw_panel = function(self,data, panel_params, coord,
                             ...
                             if(unique(data$flipped_aes) == FALSE){
                               ...
                               xlim <- range(data$ymin,data$ymax)
                               ylim <- range(data$x)
                             }
                             ...
                             for (sec in seq_along(gp)) {
                               ...
                               ptack <-
                                 ggproto_parent(GeomArcribbon, self)$draw_panel(
...
#' geom_trackribbon Function
#'
#' This is a custom geom function used to draw track ribbon plots.
#'
...
#' @param strip.label.space The spacing between track labels and track ribbon plots (default = 0.15).
...
#' @param space Specifies the spatial type for track ribbon plots (default = "free_x").
#' @param sector.gap The gap between track ribbon plots (default = 3).
#' @param polar.every Whether to use polar coordinates for every track (default = TRUE).
...
#' @param flipped_aes Whether to flip aesthetics (default = FALSE).
...
#' @import ggplot2
#' @export
geom_trackribbon <- function(mapping = NULL, data = NULL,
